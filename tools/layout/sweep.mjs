...
const DIST = path.resolve(
  ...
  opt(
    "dist",
    fs.existsSync(path.join(ROOT, "dist/docs/browser"))
      ? "dist/docs/browser"
      : "dist/docs",
  ),
);
...
const OVERLAY = {
  ...
  popover: {
    ...
    wait: ".cdk-overlay-container .cdk-overlay-pane > *",
    close: "Escape",
  },
  select: {
...
function serve(dist) {
  ...
  const server = http.createServer((req, res) => {
    ...
    if (url === "/__fixture/skeleton") {
      ...
      return res.end(skeletonFixture());
    }
...
function installInPage() {
  ...
  const identity = (el) => {
    ...
    for (const a of [
      ...
      if (v !== null) st.push(a + (v && v !== "true" ? "=" + v : ""));
    }
  ...
  const pathOf = (el) => {
    const segs = [];
    let n = el;
    while (n && n !== document.body && segs.length < 6) {
  ...
  const clipOf = (el, stopAt) => {
    let rect = { left: -1e9, top: -1e9, right: 1e9, bottom: 1e9 },
      by = null,
      kind = "visible";
    ...
    while (n && n !== document.documentElement && n !== stopAt) {
      ...
      if (ox !== "visible" || oy !== "visible") {
        const r = R(n),
          b = borders(cs);
        if (ox !== "visible") {
          rect.left = Math.max(rect.left, r.left + b.l);
          rect.right = Math.min(rect.right, r.right - b.r);
        }
        if (oy !== "visible") {
          rect.top = Math.max(rect.top, r.top + b.t);
          rect.bottom = Math.min(rect.bottom, r.bottom - b.b);
        }
        if (!by) {
          by = n;
          kind =
            [ox, oy].includes("hidden") || [ox, oy].includes("clip")
              ? "hidden"
              : "scroll";
        }
      }
  ...
  const textNodes = (root) => {
    ...
    while ((t = w.nextNode()))
      if (
        /\S/.test(t.nodeValue) &&
        t.parentElement &&
        !t.parentElement.closest("script,style")
      )
        out.push(t);
    return out;
  };
  const textRects = (t) => {
    ...
    return Array.from(rg.getClientRects()).filter(
      (r) => r.width > 0 && r.height > 0,
    );
  };
  const isFocusable = (el) =>
    el.matches(
      "button, a[href], input, select, textarea, [tabindex]:not([tabindex='-1']), [contenteditable]",
    ) &&
    !el.matches(":disabled") &&
    !el.closest("[inert]");
  const hasBoxPaint = (el) => {
    const cs = getComputedStyle(el);
    const b = borders(cs);
    const bg = cs.backgroundColor;
    const hasBg =
      bg && !/rgba\(\d+, \d+, \d+, 0\)/.test(bg) && bg !== "transparent";
    return (
      hasBg ||
      b.t + b.r + b.b + b.l > 0 ||
      el.tagName === "IMG" ||
  ...
  function sweep(rootSel, previewSel) {
    ...
    for (const c of all) {
      if (
        ...
        for (const id of c.getAttribute("aria-controls").split(/\s+/)) {
          const p = document.getElementById(id);
          if (p && seen.has(p))
            collapsed.set(p, `aria-expanded=false on ${short(c)} → #${id}`);
        }
      if (c.hasAttribute("hidden") && !collapsed.has(c))
        collapsed.set(c, "[hidden]");
      if (
        c.getAttribute("aria-hidden") === "true" &&
        c.getAttribute("role") &&
        !collapsed.has(c)
      )
        collapsed.set(c, "[aria-hidden=true][role]");
      if (c.hasAttribute("inert") && !collapsed.has(c))
        collapsed.set(c, "[inert]");
    }
    ...
    for (const c of all) {
      ...
      if (
        ...
        collapsed.set(c, "zero-height overflow:hidden wrapper");
    }
    for (const [p, reason] of collapsed) {
      ...
      for (const t of textNodes(p)) {
        ...
        for (const tr of textRects(t)) {
          const v = inter(tr, clip);
          if (v.height > visibleText) {
            visibleText = v.height;
            sampleText = t.nodeValue.trim().slice(0, 40);
          }
        }
      }
      if (cs.display === "none" || (r.height <= 0.5 && r.width <= 0.5)) {
        window.__geoClean = (window.__geoClean || 0) + 1;
        push({
          kind: "collapsed-clean",
          severity: "OK",
          element: identity(p),
          path: pathOf(p),
          reason,
      ...
      push({
        kind: "collapsed-occupies",
        severity: visibleText > 1 ? "P1" : occupies > 2 ? "P2" : "INFO",
        element: identity(p),
        path: pathOf(p),
        reason,
        box: rr(r),
        occupies_px: round(occupies),
        visible_text_px: round(visibleText),
        sample_text: sampleText,
        panel_padding: pad,
        panel_min_height: cs.minHeight,
        inner_padding: inner
          ? `${inner.paddingTop} ${inner.paddingRight} ${inner.paddingBottom} ${inner.paddingLeft}`
          : null,
        inner_min_height: inner ? inner.minHeight : null,
        grid_template_rows: cs.gridTemplateRows,
    ...
    for (const el of all) {
      if (
        !visible(el) ||
        Array.from(collapsed.keys()).some((p) => p === el || p.contains(el))
      )
        continue;
      ...
      if (hasText(el)) {
        ...
        if (clip.by) {
          for (const t of Array.from(el.childNodes).filter(
            (n) => n.nodeType === 3 && /\S/.test(n.nodeValue),
          )) {
            for (const tr of textRects(t)) {
              const cutL = Math.max(0, clip.rect.left - tr.left),
                cutR = Math.max(0, tr.right - clip.rect.right),
                cutT = Math.max(0, clip.rect.top - tr.top),
                cutB = Math.max(0, tr.bottom - clip.rect.bottom);
              ...
              if (cut > 1) {
                push({
                  kind: "text-cut",
                  fully_hidden: fullyOut,
                  severity:
                    clip.kind === "hidden"
                      ? fullyOut
                        ? "P2"
                        : cut >= tr.height * 0.5 || cutL + cutR > 4
                          ? "P1"
                          : "P2"
                      : "INFO",
                  element: identity(el),
      ...
      if (
        ...
        const r = R(el);
        const over = Math.max(0, r.right - cr.right),
          under = Math.max(0, cr.left - r.left);
        if ((over > 1 || under > 1) && r.width > 0)
          push({
            kind: "spill",
            severity: over + under > 8 ? "P1" : "P2",
            element: identity(el),
            path: pathOf(el),
            box: rr(r),
            container: short(cont),
            container_box: rr(cr),
            spill_px: { right: round(over), left: round(under) },
      ...
      if (
        ...
        push({
          kind: ellipsis ? "text-truncated" : "content-clipped",
          severity: ellipsis
      ...
      const r2 = R(el);
      if (r2.right > innerWidth + 1 && r2.width > 0 && cs.position !== "fixed")
        push({
          kind: "exceeds-viewport",
          severity: "P1",
          element: identity(el),
          path: pathOf(el),
          box: rr(r2),
          viewport: innerWidth,
          by_px: round(r2.right - innerWidth),
          target: el,
        });
    }
    ...
    if (
      ...
      push({
        kind: "page-hscroll",
        severity: inPreview ? "P1" : "INFO",
        element: inPreview ? "(component)" : "(docs chrome)",
        path: "html",
        scroll_width: document.documentElement.scrollWidth,
        client_width: cw,
        offenders: offenders.map(
          (e) =>
            `${e.closest(previewSel) ? "preview:" : "chrome:"}${short(e)} right=${round(e.getBoundingClientRect().right)}`,
        ),
      });
    }
    ...
    const boxes = all.filter(
      (el) =>
        visible(el) &&
        (hasText(el) || hasBoxPaint(el)) &&
        !el.matches(
          ".cdk-overlay-backdrop, .cdk-overlay-pane, .cdk-overlay-container, .cdk-global-overlay-wrapper, .cdk-overlay-connected-position-bounding-box",
        ) &&
        !el.closest("[aria-hidden='true'][role], [hidden], [inert]") &&
        !Array.from(collapsed.keys()).some((p) => p.contains(el)),
    );
    ...
    const rects = boxes.map((el) => {
      const r = R(el);
      const c = clipOf(el, null);
      return c.by ? inter(r, c.rect) : r;
    });
    for (let i = 0; i < boxes.length; i++)
      for (let j = i + 1; j < boxes.length; j++) {
        ...
        if (
          ...
          rects[j].width * rects[j].height === 0
        )
          continue;
        const pa = getComputedStyle(a).position,
          pb = getComputedStyle(b).position;
        const positioned =
          ["absolute", "fixed"].includes(pa) ||
          ["absolute", "fixed"].includes(pb);
        // text-node-less wrappers overlapping their siblings' children are noise: require both to paint or carry text
        push({
          kind: "overlap",
          severity: positioned ? "INFO" : "P2",
          element: identity(a),
          element_b: identity(b),
          path: pathOf(a),
          path_b: pathOf(b),
          box: rr(rects[i]),
          box_b: rr(rects[j]),
          overlap_px: { w: round(v.width), h: round(v.height) },
          positioned,
          target: a,
        });
      }
    ...
    for (const el of all) {
      ...
      if (r.width < 1 || r.height < 1) {
        const srOnly =
          ...
          cs.overflow === "hidden";
        push({
          kind: "zero-size-focusable",
          severity: srOnly ? "INFO" : "P2",
          element: identity(el),
          path: pathOf(el),
          box: rr(r),
          sr_only_pattern: srOnly,
          opacity: cs.opacity,
          target: el,
        });
      }
  ...
  function ringCut(el) {
    ...
    while (n) {
      ...
      if (cs.outlineStyle !== "none" && parseFloat(cs.outlineWidth) > 0) {
        ring = {
          ...
          via: "outline",
          spread:
            parseFloat(cs.outlineWidth) + (parseFloat(cs.outlineOffset) || 0),
        };
        break;
      }
      if (cs.boxShadow && cs.boxShadow !== "none") {
        ...
        for (const sh of cs.boxShadow.split(/,(?![^(]*\))/)) {
          ...
          if (nums.length >= 2)
            reach = Math.max(
              reach,
              Math.max(Math.abs(nums[0]), Math.abs(nums[1])) +
                (nums[2] || 0) +
                (nums[3] || 0),
            );
        }
        if (reach > 0) {
          ring = { el: n, via: "box-shadow", spread: reach };
          break;
        }
      }
      if (n === host) break;
    ...
    const r = R(ring.el);
    const rr2 = {
      left: r.left - ring.spread,
      top: r.top - ring.spread,
      right: r.right + ring.spread,
      bottom: r.bottom + ring.spread,
    };
    const clip = clipOf(ring.el, null);
    const cut = clip.by
      ? {
          left: Math.max(0, clip.rect.left - rr2.left),
          top: Math.max(0, clip.rect.top - rr2.top),
          right: Math.max(0, rr2.right - clip.rect.right),
          bottom: Math.max(0, rr2.bottom - clip.rect.bottom),
        }
    ...
    return {
      element: identity(el),
      path: pathOf(el),
      ring_on: short(ring.el),
      via: ring.via,
      spread_px: round(ring.spread),
      box: rr(r),
      clipped_by: clip.by ? short(clip.by) : null,
      clip_kind: clip.by ? clip.kind : null,
      cut_px: {
        left: round(cut.left),
        top: round(cut.top),
  ...
  window.__geo = {
    run: (rootSel, previewSel) => {
      findings.length = 0;
      const meta = sweep(rootSel, previewSel);
      return {
        meta,
        findings: findings.map((f, i) => ({ ...f, target: undefined, _i: i })),
      };
    },
    targetRect: (i) => {
      const f = findings[i];
      if (!f || !f.target) return null;
      f.target.scrollIntoView({ block: "center", inline: "nearest" });
    ...
    focusables: (sel) =>
      ...
      ).filter((el) => visible(el) && isFocusable(el)),
    ringCut,
    identity,
    previewWidth: (sel) => {
...
async function main() {
  if (!fs.existsSync(path.join(DIST, "index.html")))
    throw new Error(`No docs build at ${DIST}`);
  ...
  const browser = await pw.chromium
    ...
    .catch(async (e) => {
      ...
      if (fs.existsSync(fb))
        return pw.chromium.launch({ headless: true, executablePath: fb });
      throw e;
    });
  ...
  const meta = {
    generated_by: "tools/layout/sweep.mjs",
    tree: LABEL || gitSha,
    dist: path.relative(ROOT, DIST),
    widths: WIDTHS,
    components: components.length,
    preview_widths: {},
    checked: { elements: 0, focusables: 0, collapsed_panels: 0, text_nodes: 0 },
    notes: [],
  };
  const all = [];
        ...
        try {
          ...
          if (r && r.w >= 0 && r.h >= 0) {
            ...
            const clip = {
              ...
              width: Math.max(24, Math.min(vw, r.w + pad * 2)),
              height: Math.max(24, Math.min(vh, r.h + pad * 2)),
            };
          ...
          row.shot_error = e.message.split("\n")[0];
        }
      }
      all.push(row);
    }
  ...
  for (const component of components) {
    ...
    for (const width of WIDTHS) {
      ...
      page.on("pageerror", (e) =>
        meta.notes.push(
          `${component}/${width}: pageerror ${e.message.split("\n")[0]}`,
        ),
      );
      ...
      meta.preview_widths[`${component}@${width}`] = await page.evaluate(
        (s) => window.__geo.previewWidth(s),
        PREVIEW,
      );
      const stats = await page.evaluate((sel) => {
        ...
        while (w.nextNode()) tn++;
        return { elements: els.length, text_nodes: tn };
      }, PREVIEW);
      meta.checked.elements += stats.elements;
      meta.checked.text_nodes += stats.text_nodes;
      // default state
      const res = await page.evaluate(({ a, b }) => window.__geo.run(a, b), {
        a: PREVIEW,
        b: PREVIEW,
      });
      meta.checked.collapsed_panels += res.findings.filter(
        (f) => f.kind === "collapsed-occupies" || f.kind === "collapsed-clean",
      ).length;
      meta.checked.collapsed_clean =
        (meta.checked.collapsed_clean || 0) +
        res.findings.filter((f) => f.kind === "collapsed-clean").length;
      await record(page, component, width, "default", res);
      // focus rings
      const handles = await page.evaluateHandle(
        (sel) => window.__geo.focusables(sel),
        PREVIEW,
      );
      ...
      for (let i = 0; i < count; i++) {
        ...
        if (rc.ring === null)
          all.push({
            ...
            element: rc.element,
            path: rc.path,
            focus_visible: rc.focus_visible,
          });
        else if (rc.max_cut_px > 0.5) {
          const allSides = ["left", "top", "right", "bottom"].every(
            (k) => rc.cut_px[k] >= rc.spread_px,
          );
          const row = {
            component,
            width,
            ...
            kind: "focus-ring-cut",
            severity:
              rc.clip_kind === "hidden" ? (allSides ? "P1" : "P2") : "INFO",
            ...rc,
          };
          delete row.target;
          if (SHOTS) {
            try {
              ...
              const clip = {
                x: Math.max(0, r.x - 14),
                y: Math.max(0, r.y - 14),
                width: Math.min(vw - Math.max(0, r.x - 14), r.w + 28),
                height: Math.min(vh - Math.max(0, r.y - 14), r.h + 28),
              };
              const name = `${String(++shotN).padStart(3, "0")}-${component}-${width}-focus-ring-cut.png`;
              await page.screenshot({ path: path.join(SHOTS, name), clip });
              row.shot = name;
            } catch (e) {
              row.shot_error = e.message.split("\n")[0];
      ...
      if (ov) {
        try {
          ...
          meta.notes.push(
            `${component}/${width}: overlay not measured (${e.message.split("\n")[0]})`,
          );
          await page.keyboard.press("Escape").catch(() => {});
        }
      }
      process.stdout.write(
        `  ${width}:${all.filter((r) => r.component === component && r.width === width).length}`,
      );
      await page.close();
    }
    process.stdout.write("\n");
  ...
  console.log(
    `wrote ${path.relative(ROOT, OUT)}/findings.json${SHOTS ? " + shots in " + path.relative(ROOT, SHOTS) : ""}`,
  );
}
main().catch((e) => {
  console.error(e);
