...
const require = createRequire(
  process.env.PLAYWRIGHT_NODE_MODULES
    ? process.env.PLAYWRIGHT_NODE_MODULES + "/"
    : import.meta.url,
); // needs a playwright install (PLAYWRIGHT_NODE_MODULES=<path>/node_modules when not a project dependency)
const { chromium } = require("playwright");
const [
  BUILD,
  DIST,
  OUT,
  PLACEMENTS = "html,body",
  ONLY_IDS = "",
  OUTNAME = "matrix.json",
  SHOTS_ALL = "",
...
const OPEN = {
  // how to open the overlay in the FIRST demo
  dialog: async (p) =>
    p
      .locator(".docs-demo__preview")
      .first()
      .locator("onyx-button button")
      .first()
      .click(),
  menu: async (p) =>
    p.locator(".docs-demo__preview").first().locator("button").first().click(),
  popover: async (p) =>
    p.locator(".docs-demo__preview").first().locator("button").first().click(),
  tooltip: async (p) =>
    p.locator(".docs-demo__preview").first().locator("button").first().hover(),
  select: async (p) =>
    p
      .locator(".docs-demo__preview")
      .first()
      .locator('onyx-select [role="combobox"], onyx-select button')
      .first()
      .click(),
};
const MIME = {
...
const MEASURE = ([id, placement]) => {
  if (placement === "body") {
    ...
    for (const c of ["onyx-dark", "onyx-theme-acme"])
  ...
  for (const ss of document.styleSheets) {
    ...
    for (const r of rules)
      if (
        ...
        for (const p of r.style) if (p.startsWith("--ui-")) names.add(p);
  }
  const previews = [...document.querySelectorAll(".docs-demo__preview")];
  const hostTag = "onyx-" + id;
  let host =
    document.querySelector(".docs-demo__preview " + hostTag) ||
    previews[0]?.firstElementChild ||
    previews[0];
  ...
  for (const n of names) {
    const v = hs.getPropertyValue(n).trim();
    vars[n] = v;
    if (isColor(v)) colorNames.push(n);
  }
  probe.remove();
  ...
  const effBg = (el) => {
    ...
    return acc
      ? blend(acc, page && page.a ? page : { r: 255, g: 255, b: 255, a: 1 })
      : page && page.a
        ? page
  ...
  const pathOf = (el) => {
    ...
    for (
      ...
      e &&
      i < 4 &&
      !e.classList.contains("docs-demo__preview") &&
      e !== overlay;
      e = e.parentElement, i++
    )
      parts.unshift(
  ...
  const scope = [
    ...previews.flatMap((p) => [...p.querySelectorAll("*")]),
    ...(overlay ? [...overlay.querySelectorAll("*")] : []),
  ];
  const painted = [];
  const contrast = [];
  for (const el of scope) {
    if (painted.length > 600) break;
    ...
    painted.push({
      ...
      outline: cs.outlineStyle !== "none" ? cs.outlineColor : null,
      text: text.slice(0, 24) || null,
      inOverlay: overlay ? overlay.contains(el) : false,
    ...
    if (text) {
      ...
      if (fg)
        contrast.push({
          ...
          ratio: Math.round(ratio(fg.a < 1 ? blend(fg, b) : fg, b) * 100) / 100,
          px: parseFloat(cs.fontSize),
          weight: cs.fontWeight,
        });
    }
  }
  const cards = [...document.querySelectorAll(".docs-demo__card")]
    .slice(0, 3)
    .map((c) => c.getBoundingClientRect());
  const top = Math.min(...cards.map((c) => c.top)) + window.scrollY,
    bottom = Math.max(...cards.map((c) => c.bottom)) + window.scrollY;
  return {
...
for (const placement of PLACEMENTS.split(",")) {
  ...
  for (const th of THEMES) {
    const per = {};
    results.placements[placement][th.name] = per;
    ...
    await ctx.addInitScript(
      ([d, p]) => {
        localStorage.setItem("onyx-dark", String(d));
        localStorage.setItem("onyx-preset", p);
      },
      [th.dark, th.preset],
    );
    ...
    await page.route("http://onyx.local/**", (route) => {
      const u = new URL(route.request().url());
      let p = join(DIST, decodeURIComponent(u.pathname));
      if (!existsSync(p) || statSync(p).isDirectory()) {
        const b = join(DIST, basename(u.pathname));
        p =
          existsSync(b) && !statSync(b).isDirectory()
            ? b
            : join(DIST, "index.html");
      }
      route.fulfill({
        status: 200,
        contentType: MIME[extname(p)] || "application/octet-stream",
        body: readFileSync(p),
      });
    });
    for (const id of IDS) {
      try {
        ...
        await page.waitForSelector(".docs-demo__preview > *", {
          timeout: 20000,
          state: "attached",
        });
        ...
        if (OPEN[id]) {
          try {
            ...
            errors.push(`open(${id}): ${String(e).slice(0, 120)}`);
          }
        }
        ...
        if (placement === "html" || SHOTS_ALL) {
          const dir = join(
            OUT,
            BUILD,
            placement === "html" ? "shots" : "shots-" + placement,
            th.name,
          );
          mkdirSync(dir, { recursive: true });
          if (r.overlayChildren > 0 && OPEN[id])
        ...
        per[id] = { error: String(e).slice(0, 300) };
      }
    }
    per.__errors = errors.slice(0, 40);
    await ctx.close();
    console.log(
      `${BUILD} ${placement} ${th.name}: ${Object.keys(per).length - 1} pages, ${errors.length} console/page errors`,
    );
  }
}
