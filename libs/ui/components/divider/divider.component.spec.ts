...
describe("OnyxDividerComponent", () => {
  ...
  it("renders with role separator by default", async () => {
    const { container } = await render(`<onyx-divider></onyx-divider>`, {
  ...
  it("sets aria-orientation to vertical when orientation is vertical", async () => {
    const { container } = await render(
      `<onyx-divider [orientation]="'vertical'"></onyx-divider>`,
      { imports: [OnyxDividerComponent] },
    );
    const separator = container.querySelector('[role="separator"]');
  ...
  it("uses role presentation in decorative mode", async () => {
    const { container } = await render(
      `<onyx-divider [decorative]="true"></onyx-divider>`,
      { imports: [OnyxDividerComponent] },
    );
  ...
  it("renders slotted label content", async () => {
    await render(`<onyx-divider>OR</onyx-divider>`, {
      imports: [OnyxDividerComponent],
    });
  ...
  it("applies has-label class when label content is provided via ng-content slot", async () => {
    ...
    const { container } = await render(`<onyx-divider>OR</onyx-divider>`, {
      imports: [OnyxDividerComponent],
    });
